...
public class Main {
	...
	final double REFRESH_TIME = 10.0; // Refresh displayed stats every 10 seconds of events.
	final double PING_TIME = 1; // Will ping all the machine needs in 1 second intervals.
	...
	public Main(int sizeX, int sizeZ){
		...
		// Generate the blocks that we will be getting from teh quarry ahead of time.
	...
	/*
	 * Calculates the power needed by all the machines and then turns on generators as needed.
	 */
	private void calcPower(double t){
		// We will have to go through all the paths and check for their power usage.
		double totalRF = 0.0;
		double totalEU = 0.0;
		for(Path p : paths){
	...
	public void start(){
		while(!pq.isEmpty()){
			// We will remove the event and then run code based on what has just happened.
			Event e = pq.remove();
			
			// Which event occured and do appropriate action
			switch(e.type){
			case GEN_FINISHED:
				// This event will occur when a generator has finished operating on its
				// current piece of coal. We should recalculate if we need that generator or not.
				// We may just turn the generator off until next ping.
				generatorsRunningEU--;
				generatorsRunningRF--;
				break;
			case BLOCK_ARRIVED:
				// This means the quarry dug up a block from the ground and we should deal with it.
				// Put the block in its place and move on.
				
				break;
			case MACH_FINISHED:
				// This means a machine somewhere in the system has finished its process.
				// Let the path it is contained in know of this and continue the simulation.
				
				break;
			case PING:
				// This is an annual check for energy usage and turning on generators if needed.
			...
			case REFRESH:
				...
				break;
			case QUARRY_END:
				// This tells the simulation has finished with quarry stuff and that we should
